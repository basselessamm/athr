...
class SearchNotifier extends StateNotifier<SearchState> {
  ...
  Future<void> _performSearch(String query) async {
    ...
    try {
      ...
      // 2. Search Quran in memory using the same shared normalizer so all
      // three tabs behave identically for the same query.
      ...
      if (normalizedQuery.isNotEmpty) {
        for (int surah = 1; surah <= 114; surah++) {
          ...
          for (int ayah = 1; ayah <= count; ayah++) {
            ...
            final normalizedVerse = normalizeArabic(verse.text);
            if (normalizedVerse.contains(normalizedQuery)) {
              quranResults.add(
                QuranSearchResult(surah: surah, ayah: ayah, text: verse.text),
              );
              if (quranResults.length >= 100) {                 break; } // Limit to 100 to avoid UI freeze on common words
            }
          }
